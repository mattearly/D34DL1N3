...
public:
  ...
  Date(int mm, int dd, int yyyy)
      : month(mm), day(dd), year(yyyy), hour(0), minute(1) {}
  Date(int mm, int dd, int yyyy, int hh, int mi)
      : month(mm), day(dd), year(yyyy), hour(hh), minute(mi) {}

  // Non-initilized version
  Date() : month(-1), day(-1), year(-1), hour(-1), minute(-1) {}
  ~Date() {}
