...
void RunTasks::exec() {
  ...
  do {
    ...
    case 6:
      ...
      break;
    case 7:
      choice = -1; // to hit default on next loop and display error message
      break;
    case 8:
      choice = -1;
      break;
...
int RunTasks::menu(pair<int, int> &__minmax) {
  ...
  cout << "\n ->" << latestMessage << "\n\n"
       ...
       << left << setw(16) << "1. Add Task" << " | " << "6. Show History"
       << endl
...
void RunTasks::createNewTask() {
  ...
  do { 
    ...
    tmpname = reduce(tmpname); // tighen her up
           ...
           1); // cant imagine 1 character being enough to desribe a task...
  ...
  cout << "\nEnter month: ";
  if (tmpyear == 1900 + ltm->tm_year)
    tmpmonth = getNumber(1 + ltm->tm_mon,
                         12); // limit this year to the months still left
  else
    tmpmonth = getNumber(1, 12); // otherwise future years any month is fine

  cout << "\nEnter day: ";
  if (tmpyear == 1900 + ltm->tm_year && tmpmonth == 1 + ltm->tm_mon) {
    tmpday = getNumber(ltm->tm_mday, 31); // limit to the number of days left in
                                          // the month if task is this month
  } else {
    tmpday =
        getNumber(1, 31); // otherwise accept any date (always goes to 31, which
                          // needs fixed and set based on days in month)
  }
...
void RunTasks::createNewTask(vector<Task> &inctasks) { allTasks = inctasks; }
...
void RunTasks::viewAllTasks() {
  ...
  for (auto &it : allTasks) {
    cout << it.getName() << "\n   Due: " << it.getMonth() << "/" << it.getDay()
         << "/" << it.getYear() << " @ " << setw(2) << std::setfill('0')
         << right << it.getHour() << ":" << setw(2) << std::setfill('0')
         << right << it.getMinute() << endl;
  }
  pressEnterToContinue();
}
...
void RunTasks::viewHighestPriorityTask() {
  ...
  if (allTasks.size() < 1) {
    ...
    cout << hour << ":" << setw(2) << std::setfill('0') << right
         << allTasks[0].getMinute() << AM_PM << endl;
    ...
    for (i = 0; i < listsize - 1; i++) {
      if (i == 0 &&
          !(allTasks[i].getRawDate() < allTasks[i + 1].getRawDate())) {
        highestpri = i + 1;
      } else if (!(allTasks[highestpri].getRawDate() <
                   allTasks[i + 1].getRawDate())) {
        highestpri = i + 1;
      }
    }
    ...
    cout << hour << ":" << setw(2) << std::setfill('0') << right
         << allTasks[highestpri].getMinute() << AM_PM << std::endl;
    // end display due time
      ...
      if (i == highestpri) {
        ...
        if (allTasks[i].getMonth() == allTasks[highestpri].getMonth()) {
          if (allTasks[i].getDay() == allTasks[highestpri].getDay()) {
            cout << count << ". " << allTasks[i].getName();

            // display time of this tasks
            cout << " @";
            string AM_PM = "AM";
            ...
            cout << hour << ":" << setw(2) << std::setfill('0') << right
                 << allTasks[i].getMinute() << AM_PM << endl;
            // end display due time
            count++;
          }
        }
    ...
    cout << "    Due: " << allTasks[highestpri].getMonth() << "/"
         ...
         << allTasks[highestpri].getYear() << std::endl;
...
void RunTasks::printCurrentTime() {
  ...
  cout << std::setfill('0');
  cout << "     ";
  cout << setw(2) << right << hour << ":" << setw(2) << right << ltm->tm_min
       << AM_PM << "  ";
  //		 << setw(2) << 1 + ltm->tm_sec << " ";  //no seconds
  // print date
  cout << setw(2) << right << 1 + ltm->tm_mon << "/" << setw(2) << right
       << ltm->tm_mday << "/" << setw(4) << 1900 + ltm->tm_year << endl;
  cout << std::setfill(' ');
}
...
void RunTasks::removeTask() {
  ...
  for (auto &it : allTasks) {
    ...
    cout << "\n"
         << _count << ". " << it.getName() << "\n    Due: " << it.getMonth()
         << "/" << it.getDay() << "/" << it.getYear() << " @ " << setw(2)
         << std::setfill('0') << right << it.getHour() << ":" << setw(2)
         << std::setfill('0') << right << it.getMinute() << endl;
  }
  cout << "\nWhich would you like to remove (by number): ";
  _choice = getNumber(1, _count);
  addTaskToPendingHistory(allTasks[_choice - 1]);
  allTasks.erase(allTasks.begin() - 1 + _choice);
  cout << "\nRemoval complete\n";
  pressEnterToContinue();
}

void RunTasks::addTaskToPendingHistory(Task &old_task) {
  // update time
  now = time(0);
  tm *ltm = localtime(&now);
  ...
  //      << AM_PM << "  ";
  cout << "attempting history thing ";
  cout << "on" << old_task.getName() << endl;

  string o_task;

  o_task = old_task.getName() + "->removed: ";
  o_task += to_string(1 + ltm->tm_mon) + "/";
  o_task += to_string(ltm->tm_mday) + "/";
  o_task += to_string(1900 + ltm->tm_year);

  cout << "history look: " << o_task << endl;

  pendingHistory.push_back(o_task);
}

void RunTasks::showHistory() {
  std::string home(user_dir);

  ifstream hin(home + "/.dtpriority/history");
  if (!hin) {
...
void RunTasks::editTask() {
  ...
  case 2: {
    int tmpmonth;
    cout << "\nEnter month: ";
    if (allTasks[_choice - 1].getYear() == 1900 + ltm->tm_year)
      tmpmonth = getNumber(1 + ltm->tm_mon,
                           12); // limit this year to the months still left
    else
      tmpmonth = getNumber(1, 12); // otherwise future years any month is fine
  ...
  case 3: {
    ...
    cout << "\nEnter day: ";
    if (allTasks[_choice - 1].getYear() == 1900 + ltm->tm_year &&
        allTasks[_choice - 1].getMonth() == 1 + ltm->tm_mon) {
      tmpday =
          getNumber(ltm->tm_mday, 31); // limit to the number of days left in
                                       // the month if task is this month
    } else {
      tmpday =
          getNumber(1, 31); // otherwise accept any date (always goes to 31,
                            // which needs fixed and set based on days in month)
    }
